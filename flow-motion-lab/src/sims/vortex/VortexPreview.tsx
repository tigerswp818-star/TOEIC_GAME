...
import { tangentialSpeed, angularSpeed } from "./vortexModel";
import { velocityColor } from "@/lib/colors";
import { clamp } from "@/lib/math";

const TANK = 2; // metres
const OMEGA = 2.4; // rad/s
const R_CORE = 0.7; // metres (≈ 0.35 × tank)
const VMAX = tangentialSpeed(R_CORE, OMEGA, R_CORE);
...
/** Looping top-down mini view of swirling particles for the dashboard card. */
export default function VortexPreview() {
  const draw = ({ ctx, width, height, dt, theme }: DrawContext) => {
    ...
    // faint concentric guide rings (the vortex streamlines)
